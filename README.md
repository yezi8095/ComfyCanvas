# ComfyCanvas（亿幕画布）（测试阶段）

这是亿幕无限画布桌面应用的最新可开发源码。当前发布版本为 `v0.2.0`；仓库保留继续修改、安装依赖和重新构建所需的文件，不包含历史备份、构建缓存或可重新生成的产物。
...
`v0.2.0` 的安装包与便携 ZIP 发布在 GitHub Releases，更新内容见 [v0.2.0 版本说明](./docs/版本说明-0.2.0.md)。
