...
async fn discover_api_models(endpoint: String, api_key: String) -> Result<Vec<String>, String> {
    ...
    let response = request
        .send()
        .await
        .map_err(|e| format!("无法连接模型接口：{e}"))?;
    let status = response.status();
    let value: serde_json::Value = response
        .json()
        .await
        .map_err(|e| format!("模型接口返回的不是 JSON：{e}"))?;
...
async fn generate_alibaba_wan_video(
    ...
    let response = client
        ...
        .send()
        .await
        .map_err(|error| format!("无法连接阿里百炼：{error}"))?;
    let status = response.status();
    let created: serde_json::Value = response
        .json()
        .await
        .map_err(|error| format!("无法读取阿里百炼响应：{error}"))?;
    if !status.is_success() {
        let message = created
            .pointer("/message")
            .and_then(serde_json::Value::as_str)
    ...
    for _ in 0..240 {
        ...
        let task: serde_json::Value = response
            .json()
            .await
            .map_err(|error| format!("无法读取阿里百炼任务状态：{error}"))?;
...
async fn generate_kling_video(
    ...
    let is_image = mode != "text" && !image_urls.is_empty();
    let route = if is_image {
        "image2video"
    } else {
        "text2video"
    };
    ...
    let mut payload = serde_json::json!({
      ...
      "mode": if quality.eq_ignore_ascii_case("1080P") { "pro" } else { "std" },
      "aspect_ratio": ratio, "duration": if duration >= 10 { "10" } else { "5" }
    });
    if is_image {
        let value = image_urls[0]
            .split_once(',')
            .map(|(_, encoded)| encoded)
            .unwrap_or(image_urls[0].as_str());
        payload["image"] = serde_json::Value::String(value.to_string());
    }
    let client = reqwest::Client::builder()
        .timeout(Duration::from_secs(45))
        .build()
    ...
    let created: serde_json::Value = response
        .json()
        .await
        .map_err(|error| format!("无法读取可灵响应：{error}"))?;
    if !status.is_success()
        || created
            .get("code")
            ...
            != 0
    {
        let message = created
            ...
            .and_then(serde_json::Value::as_str)
            .unwrap_or("请求被平台拒绝");
        return Err(format!("可灵创建任务失败（HTTP {status}）：{message}"));
    }
    let task_id = created
    ...
    for _ in 0..240 {
        ...
        let response = client
            ...
            .send()
            .await
            .map_err(|error| format!("查询可灵任务失败：{error}"))?;
        let status = response.status();
        let task: serde_json::Value = response
            .json()
            .await
            .map_err(|error| format!("无法读取可灵任务状态：{error}"))?;
        if !status.is_success() {
            return Err(format!("可灵查询任务失败（HTTP {status}）：{task}"));
        }
        let task_status = task
...
async fn generate_doubao_video(
    ...
    let created: serde_json::Value = response
        .json()
        .await
        .map_err(|error| format!("无法读取豆包响应：{error}"))?;
    if !status.is_success() {
        let message = created
            .pointer("/error/message")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("请求被平台拒绝");
        return Err(format!("豆包创建任务失败（HTTP {status}）：{message}"));
    }
    let task_id = created
        .get("id")
        .and_then(serde_json::Value::as_str)
    ...
    for _ in 0..240 {
        ...
        let response = client
            ...
            .send()
            .await
            .map_err(|error| format!("查询豆包任务失败：{error}"))?;
        let status = response.status();
        let task: serde_json::Value = response
            .json()
            .await
            .map_err(|error| format!("无法读取豆包任务状态：{error}"))?;
        if !status.is_success() {
            let message = task
                .pointer("/error/message")
                .and_then(serde_json::Value::as_str)
                .unwrap_or("查询任务被平台拒绝");
            return Err(format!("豆包查询任务失败（HTTP {status}）：{message}"));
        }
        let task_status = task
            .get("status")
            .and_then(serde_json::Value::as_str)
...
async fn rewrite_alibaba_prompt(
    ...
    let response = client
        ...
        .send()
        .await
        .map_err(|e| format!("无法连接阿里百炼：{e}"))?;
    let status = response.status();
    let body: serde_json::Value = response
        .json()
        .await
        .map_err(|e| format!("无法读取阿里百炼响应：{e}"))?;
    if !status.is_success() {
        let message = body
            .pointer("/error/message")
            .and_then(serde_json::Value::as_str)
            .or_else(|| body.pointer("/message").and_then(serde_json::Value::as_str))
            .unwrap_or("平台拒绝了请求");
